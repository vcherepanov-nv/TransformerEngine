...
std::vector<py::object> fused_attn_fwd(
  ...
  void *score_mod_ptr = score_mod.is_none() ? nullptr : score_mod.ptr();
  NVTE_SCOPED_GIL_RELEASE({
    nvte_fused_attn_fwd(
        ...
        softmax_type, window_size[0], window_size[1], bottom_right_diagonal, score_mod_ptr,
...
std::vector<py::object> fused_attn_bwd(
    ...
    py::object score_mod, py::object score_mod_bprop) {
  auto none = py::none();
  ...
  // populate tensors with appropriate shapes and dtypes
  void *score_mod_ptr = score_mod.is_none() ? nullptr : score_mod.ptr();
  void *score_mod_bprop_ptr = score_mod_bprop.is_none() ? nullptr : score_mod_bprop.ptr();
  NVTE_SCOPED_GIL_RELEASE({
    nvte_fused_attn_bwd(
        ...
        window_size[1], bottom_right_diagonal, deterministic, cuda_graph, score_mod_ptr,
        score_mod_bprop_ptr, workspace.data(), at::cuda::getCurrentCUDAStream());
  });
