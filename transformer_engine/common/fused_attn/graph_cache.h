...
// Implemented by the f16 fused-attention translation unit and shared by every
// graph-cache specialization.
bool is_graph_build_profiling_enabled();
uint64_t get_graph_cache_epoch();
...
inline std::string graph_build_profile_range(const char *stage, Pass pass) {
  return std::string("cudnn_graph_") + stage + "_cpp_" + pass_name(pass);
}
...
inline void query_support(Backend backend, Pass pass, cudnn_frontend::graph::Graph &graph,
                          ...
                          std::optional<nvtx::NVTXWrapper> *materialization_range = nullptr) {
  using graph_cache_debug::BuildStage;

  auto run = [&](BuildStage stage, const char *call_name, const char *profile_stage, auto &&call) {
    std::optional<nvtx::NVTXWrapper> stage_range;
    if (is_graph_build_profiling_enabled()) {
      stage_range.emplace(graph_build_profile_range(profile_stage, pass));
    }
  ...
  run(BuildStage::Validate, "validate", "native_validation", [&] { return graph.validate(); });
  if (materialization_range != nullptr) materialization_range->reset();
  run(BuildStage::BuildOpGraph, "build_operation_graph", "build_operation_graph",
      [&] { return graph.build_operation_graph(handle); });
  run(BuildStage::CreatePlans, "create_execution_plans", "create_execution_plans",
      [&] { return graph.create_execution_plans({cudnn_frontend::HeurMode_t::A}); });
  run(BuildStage::CheckSupport, "check_support", "check_support",
...
std::shared_ptr<CacheEntry<GraphAndTensors>> cache_graph(GraphCache<GraphAndTensors> &cache,
                                                         ...
                                                         std::optional<nvtx::NVTXWrapper>
                                                             *total_profile_range = nullptr) {
  ...
  auto find = [&]() -> std::shared_ptr<CacheEntry<GraphAndTensors>> {
    ...
    current_epoch = get_graph_cache_epoch();
  ...
  std::optional<nvtx::NVTXWrapper> materialization_range;
  std::optional<nvtx::NVTXWrapper> definition_range;
  if (is_graph_build_profiling_enabled()) {
    if (total_profile_range != nullptr) {
      total_profile_range->emplace(graph_build_profile_range("build", pass));
    }
    materialization_range.emplace(
        graph_build_profile_range("materialization_and_validation", pass));
    definition_range.emplace(graph_build_profile_range("native_definition", pass));
  }
  ...
  bool inserted = false;
  {
    ...
    if (cache.epoch == current_epoch) {
      ...
      // A reset raced this build. Return the usable graph to this caller, but
      // leave it uncached so the next call observes the reset.
...
auto get_graph(const FusedAttnConfig &cfg, cudnnHandle_t handle,
               std::optional<nvtx::NVTXWrapper> *total_profile_range = nullptr) {
  ...
  return detail::cache_graph(cache, cfg.make_cache_key(kPass), kBackend, kPass, handle,
                             [&] { return kCreateGraphFn(cfg); }, total_profile_range);
}
...
void build_plans(Backend backend, Pass pass, CacheEntry<GraphAndTensors> &entry) {
  std::call_once(entry.build_plans_once, [&] {
    std::optional<nvtx::NVTXWrapper> stage_range;
    if (is_graph_build_profiling_enabled()) {
      stage_range.emplace(detail::graph_build_profile_range("build_plans", pass));
    }
