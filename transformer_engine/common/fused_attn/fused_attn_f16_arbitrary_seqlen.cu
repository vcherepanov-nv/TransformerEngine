...
std::atomic<uint64_t> graph_cache_epoch{0};
...
bool is_graph_build_profiling_enabled() {
  return graph_build_profiling_enabled.load();
}

uint64_t get_graph_cache_epoch() {
  return graph_cache_epoch.load();
}
...
void fused_attn_arbitrary_seqlen_fwd_impl(
  ...
  try {
    std::optional<nvtx::NVTXWrapper> total_profile_range;
    auto cache_entry = get_graph<Backend::F16, Pass::Fwd, create_graph_f16_fwd>(
        cfg, handle, &total_profile_range);
...
void fused_attn_arbitrary_seqlen_bwd_impl(
  ...
  try {
    std::optional<nvtx::NVTXWrapper> total_profile_range;
    auto cache_entry = get_graph<Backend::F16, Pass::Bwd, create_graph_f16_bwd>(
        cfg, handle, &total_profile_range);
