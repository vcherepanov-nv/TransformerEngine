...
constexpr int32_t kNvteDLCPU = 1;
constexpr int32_t kNvteDLCUDA = 2;
constexpr int32_t kNvteDLCUDAHost = 3;
constexpr int32_t kNvteDLCUDAManaged = 13;
constexpr uint8_t kNvteDLInt = 0;
constexpr uint8_t kNvteDLUInt = 1;
constexpr uint8_t kNvteDLFloat = 2;
constexpr uint8_t kNvteDLBfloat = 4;
constexpr uint8_t kNvteDLBool = 6;
...
struct NvteDLTensor {
  ...
  NvteDLDevice device;
  int32_t ndim;
  NvteDLDataType dtype;
  int64_t *shape;
  int64_t *strides;
  uint64_t byte_offset;
};

struct NvteDLManagedTensor {
  NvteDLTensor dl_tensor;
  void *manager_ctx;
  void (*deleter)(NvteDLManagedTensor *self);
};

struct DlpackTensorView {
  PyObject *capsule = nullptr;
  NvteDLManagedTensor *managed = nullptr;

  DlpackTensorView() = default;
  DlpackTensorView(PyObject *capsule_obj, NvteDLManagedTensor *managed_tensor)
...
cudnn_frontend::DataType_t convert_dlpack_dtype(const NvteDLDataType &dtype) {
  switch (dtype.code) {
    case kNvteDLUInt:
      if (dtype.bits == 8) {
...
decltype(auto) with_dlpack_tensor(py::handle tensor_obj, Fn &&fn) {
  ...
  auto *managed = static_cast<NvteDLManagedTensor *>(
      PyCapsule_GetPointer(capsule.ptr(), kDlpackCapsuleName));
...
TensorAttr create_tensor_attr_from_dlpack(
  ...
  return with_dlpack_tensor(tensor_obj, [&](const NvteDLManagedTensor &managed) -> TensorAttr {
    const auto device_type = managed.dl_tensor.device.device_type;
    NVTE_CHECK(device_type == kNvteDLCPU || device_type == kNvteDLCUDAHost ||
                   device_type == kNvteDLCUDA || device_type == kNvteDLCUDAManaged,
    ...
    auto props = cudnn_frontend::graph::Tensor_attributes()
                     ...
                     .set_is_pass_by_value(device_type == kNvteDLCPU)
                     .set_dim(dims);
...
std::uint64_t get_extra_tensor_signature(void *extra_tensors_ptr) {
  ...
  for (const auto &name : get_sorted_extra_tensor_names(extra_tensors)) {
    ...
    with_dlpack_tensor(tensor_obj, [&](const NvteDLManagedTensor &managed) {
...
auto make_attention_score_modifier(void *callback_ptr, void *extra_tensors_ptr,
                                   ExtraTensorList *extra_tensor_attrs)
    -> std::function<TensorAttr(std::shared_ptr<cudnn_frontend::graph::Graph>, TensorAttr)> {
  ...
  return [callback, extra_tensors_ptr, extra_tensor_attrs](
             ...
             TensorAttr score_tensor) mutable {
    py::gil_scoped_acquire gil;
...
void fused_attn_arbitrary_seqlen_fwd_impl(
    ...
    int64_t window_size_right, bool bottom_right_diagonal, void *score_mod,
    void *score_mod_tensors, void *devPtrQ, void *devPtrK, void *devPtrV, void *devPtrBias,
...
void fused_attn_arbitrary_seqlen_bwd_impl(
    ...
    void *devPtrBias, void *devPtrSoftmaxOffset, void *devPtrdQ, void *devPtrdK,
    void *devPtrdV, void *devPtrdO, void *devPtrdBias, void *devPtrdSoftmaxOffset,
    void *devPtrDropoutSeed, void *devPtrDropoutOffset, void *devPtrCuSeqlensQ,
...
void fused_attn_arbitrary_seqlen_fwd(
    ...
    NVTE_Softmax_Type softmax_type, int64_t window_size_left, int64_t window_size_right,
    bool bottom_right_diagonal, void *score_mod, void *score_mod_tensors, const Tensor *input_Q,
  ...
  fused_attn_arbitrary_seqlen_fwd_impl(
      ...
      score_mod, score_mod_tensors, devPtrQ, devPtrK, devPtrV, devPtrBias, devPtrSoftmaxOffset,
      devPtrS1, devPtrS2, devPtrO, devPtrDropoutSeed, devPtrDropoutOffset, devPtrCuSeqlensQ,
...
void fused_attn_arbitrary_seqlen_bwd(
  ...
  fused_attn_arbitrary_seqlen_bwd_impl(
      ...
      window_size_left, window_size_right, bottom_right_diagonal, deterministic, score_mod,
      score_mod_bprop, score_mod_tensors, score_mod_bprop_tensors, devPtrQ, devPtrK, devPtrV,
      devPtrO, devPtrSoftmaxStats, devPtrBias, devPtrSoftmaxOffset, devPtrdQ, devPtrdK, devPtrdV,
      devPtrdO, devPtrdBias, devPtrdSoftmaxOffset, devPtrDropoutSeed, devPtrDropoutOffset,
