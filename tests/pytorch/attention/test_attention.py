...
def test_dpa_score_mod_causal(dtype, model_configs, model):
    """Test DotProductAttention with score_mod=causal mask.
    """
    config = model_configs[model]
    qkv_layout = "bshd_bshd_bshd"
    qkv_format = "bshd"
    ...
    pygraph = cudnn.pygraph(
        io_data_type=cudnn.data_type.HALF,
        intermediate_data_type=cudnn.data_type.FLOAT,
        compute_data_type=cudnn.data_type.FLOAT,
    )
    neg_inf_tensor_cpu = torch.full((1, 1, 1, 1), float("-inf"))
    neg_inf_tensor = pygraph.tensor(
        name="neg_inf_scalar",
    ...
    def causal_mask_score_mod(graph, q_kt_tensor):
            row_index = graph.gen_index(input=q_kt_tensor, axis=2)
            row_index.set_data_type(cudnn.data_type.INT32)

            col_index = graph.gen_index(input=q_kt_tensor, axis=3)
            col_index.set_data_type(cudnn.data_type.INT32)

            mask = graph.cmp_ge(input=row_index, comparison=col_index, compute_data_type=cudnn.data_type.BOOLEAN)
            mask.set_data_type(cudnn.data_type.BOOLEAN)

            out = graph.binary_select(input0=q_kt_tensor, input1=q_kt_tensor, mask=mask)
            print("CAUSAL MASK!!!")
            return out
    ...
    out_sm = block(q, k, v, qkv_format=qkv_format, cu_seqlens_q=cu_seqlens, cu_seqlens_kv=cu_seqlens,
                   max_seqlen_q=sq, max_seqlen_kv=sq, attn_mask_type=config.attn_mask_type,
                   score_mod=causal_mask_score_mod)
    out_sm.backward(out_grad)
    dq_sm = q.grad.clone()
    dk_sm = k.grad.clone()
    dv_sm = v.grad.clone()
